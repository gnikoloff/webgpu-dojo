  ...
  throw error;
    ...
    function getLibraryUrl(library, moduleName, options) {
      ...
      if (options.CDN) {
        ...
        return `${options.CDN}/${moduleName}@${VERSION$2}/dist/libs/${library}`;
      }
...
${String.fromCharCode(dataView.getUint8(byteOffset + 3))}`;
    ...
    function parseGLTFContainerSync(gltf, data, byteOffset, options) {
      ...
      if (typeof data === 'string') {
        ...
        assert$2(glb.type === 'glTF', `Invalid GLB magic string ${glb.type}`);
        gltf._glb = glb;
        gltf.json = glb.json;
      } else {
...
`;
    class GridHelper extends Mesh {
        constructor(device, radius, divisionsCount, color = [0.5, 0.5, 0.5, 1]) {
...
`;
    const getVertexShaderSnippet = ({ includeNormals = true, includeUvs = true, } = {}) => `
...
`;
    ...
    (() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        meshColorFolder.addColor(OPTIONS, 'ambientColor').onChange((v) => {
            const ambientColor = new Float32Array(normalizeRGB(parse$3(v).values));
        ...
        meshColorFolder.addColor(OPTIONS, 'diffuseColor').onChange((v) => {
            const diffuseColor = new Float32Array(normalizeRGB(parse$3(v).values));
        ...
        meshColorFolder.addColor(OPTIONS, 'specularColor').onChange((v) => {
            const specularColor = new Float32Array(normalizeRGB(parse$3(v).values));
        ...
        const perspCamera = new PerspectiveCamera((45 * Math.PI) / 180, canvas.width / canvas.height, 0.1, 200);
        ...
        const sharedUniforms = {
            ...
            ambientColor: {
                type: 'vec4<f32>',
                value: new Float32Array(normalizeRGB(parse$3(OPTIONS.ambientColor).values)),
            },
            diffuseColor: {
                type: 'vec4<f32>',
                value: new Float32Array(normalizeRGB(parse$3(OPTIONS.diffuseColor).values)),
            },
            specularColor: {
                type: 'vec4<f32>',
                value: new Float32Array(normalizeRGB(parse$3(OPTIONS.specularColor).values)),
            },
            lightPosition: {
                type: 'vec3<f32>',
        ...
        const duckGLTF = yield load(`${window['ASSETS_BASE_URL']}/Duck.gltf`, GLTFLoader);
        const duckGeometry = new Geometry(device);
        {
            const indexBuffer = new IndexBuffer(device, duckGLTF.meshes[0].primitives[0].indices.value);
            const vertexBuffer = new VertexBuffer(device, 0, duckGLTF.meshes[0].primitives[0].attributes.POSITION.value, 3 * Float32Array.BYTES_PER_ELEMENT).addAttribute('position', 0, 3 * Float32Array.BYTES_PER_ELEMENT, 'float32x3');
            const normalBuffer = new VertexBuffer(device, 1, duckGLTF.meshes[0].primitives[0].attributes.NORMAL.value, 3 * Float32Array.BYTES_PER_ELEMENT).addAttribute('normal', 0, 3 * Float32Array.BYTES_PER_ELEMENT, 'float32x3');
            const uvBuffer = new VertexBuffer(device, 2, duckGLTF.meshes[0].primitives[0].attributes.TEXCOORD_0.value, 2 * Float32Array.BYTES_PER_ELEMENT).addAttribute('uv', 0, 2 * Float32Array.BYTES_PER_ELEMENT, 'float32x2');
            duckGeometry
                .addVertexBuffer(vertexBuffer)
        ...
        {
            const { vertices, normal, indices } = index.createBox();
            const vertexBuffer = new VertexBuffer(device, 0, vertices, 3 * Float32Array.BYTES_PER_ELEMENT).addAttribute('position', 0, 3 * Float32Array.BYTES_PER_ELEMENT, 'float32x3');
            const normalBuffer = new VertexBuffer(device, 1, normal, 3 * Float32Array.BYTES_PER_ELEMENT).addAttribute('normal', 0, 3 * Float32Array.BYTES_PER_ELEMENT, 'float32x3');
